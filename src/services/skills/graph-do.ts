...
import type { Env, NodeRow, EdgeRow, AdjacencyRow, NodeProps, EdgeProps, CreateNodeBody, UpdateNodeBody, CreateEdgeBody, UpdateEdgeBody, TraverseBody, PathsBody, SubgraphBody, TraversalNode, PathResult, SubgraphResult } from '@/types';

const SCHEMA_STMTS = [
  `CREATE TABLE IF NOT EXISTS nodes ( id TEXT PRIMARY KEY, label TEXT NOT NULL DEFAULT '', properties TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')) )`,
  `CREATE TABLE IF NOT EXISTS edges ( id TEXT PRIMARY KEY, from_id TEXT NOT NULL, to_id TEXT NOT NULL, type TEXT NOT NULL DEFAULT '', properties TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT (datetime('now')) )`,
  `CREATE TABLE IF NOT EXISTS adjacency ( node_id TEXT NOT NULL, neighbour_id TEXT NOT NULL, edge_id TEXT NOT NULL, direction TEXT NOT NULL, edge_type TEXT NOT NULL DEFAULT '', PRIMARY KEY (node_id, edge_id, direction) )`,
...
function rowToNode(r: NodeRow): NodeProps { return { id: r.id, label: r.label, properties: JSON.parse(r.properties), createdAt: r.created_at, updatedAt: r.updated_at }; }
function rowToEdge(r: EdgeRow): EdgeProps { return { id: r.id, fromId: r.from_id, toId: r.to_id, type: r.type, properties: JSON.parse(r.properties), createdAt: r.created_at }; }
...
export class SkillsGraphDO extends DurableObject<Env> {
  private sql: SqlStorage;

  constructor(ctx: DurableObjectState, env: Env) {
    super(ctx, env);
    this.sql = ctx.storage.sql;
    ctx.blockConcurrencyWhile(async () => { for (const stmt of SCHEMA_STMTS) { this.sql.exec(stmt); } });
  }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (path.match(/^\/nodes\/[^/]+\/neighbours$/) && method === 'GET') return this.getNeighbours(seg(path, 1), url);
      ...
      if (path.match(/^\/skills\/[^/]+\/subgraph$/) && method === 'GET') return this.getSkillsSubgraph(seg(path, 1));
      if (path.match(/^\/skills\/[^/]+\/dependencies$/) && method === 'GET') return this.getDependencies(seg(path, 1));
  ...
  private getDependencies(nodeId: string): Response {
    if (!nodeId) return err('nodeId is required');
    const adj = this.queryAdjacency(nodeId, 'out', ['DEPENDS_ON'], 1000);
    if (!adj.length) return ok({ dependencies: [] });
    const neighbourIds = adj.map(a => a.neighbour_id);
    const nodeRows = this.fetchByIds<NodeRow>('nodes', neighbourIds);
    return ok({ dependencies: nodeRows.map(rowToNode) });
  }

  private getSkillsSubgraph(nodeId: string): Response {
    if (!nodeId) return err('nodeId is required');
    ...
    return ok({ nodes: nodeRows.map(rowToNode), edges: edgeRows.map(rowToEdge) });
  }
  ...
  private createNode(body: CreateNodeBody): Response {
    const { id = uuid(), label = '', properties = {} } = body;
    const nStr = now();
    this.sql.exec('INSERT INTO nodes (id, label, properties, created_at, updated_at) VALUES (?, ?, ?, ?, ?)', id, label, JSON.stringify(properties), nStr, nStr);
    return ok({ id, label, properties, createdAt: nStr, updatedAt: nStr }, 201);
  }
  ...
  private updateNode(id: string, body: UpdateNodeBody): Response {
    ...
    if (!rows.length) return err('Node not found', 404);
    const nStr = now();
    const current = rowToNode(rows[0]);
    const newLabel = label ?? current.label;
    const newProps = properties ? { ...current.properties, ...properties } : current.properties;
    this.sql.exec('UPDATE nodes SET label = ?, properties = ?, updated_at = ? WHERE id = ?', newLabel, JSON.stringify(newProps), nStr, id);
    return ok({ id, label: newLabel, properties: newProps, createdAt: current.createdAt, updatedAt: nStr });
  }
  ...
  private listEdges(url: URL): Response {
    ...
    if (fromId) { sql += ' AND from_id = ?'; params.push(fromId); }
    if (toId) { sql += ' AND to_id = ?'; params.push(toId); }
    sql += ' ORDER BY created_at DESC LIMIT ? OFFSET ?';
    params.push(limit, offset);
  ...
  private createEdge(body: CreateEdgeBody): Response {
    const { id = uuid(), fromId, toId, type = '', properties = {} } = body;
    if (!fromId || !toId) return err('fromId and toId required');

    const fromRows = [...this.sql.exec<NodeRow>('SELECT id FROM nodes WHERE id = ?', fromId)];
    const toRows   = [...this.sql.exec<NodeRow>('SELECT id FROM nodes WHERE id = ?', toId)];
    if (!fromRows.length || !toRows.length) return err('Source or target node not found', 404);

    const nStr = now();
    this.sql.exec('INSERT INTO edges (id, from_id, to_id, type, properties, created_at) VALUES (?, ?, ?, ?, ?, ?)', id, fromId, toId, type, JSON.stringify(properties), nStr);
    this.sql.exec('INSERT INTO adjacency (node_id, neighbour_id, edge_id, direction, edge_type) VALUES (?, ?, ?, ?, ?)', fromId, toId, id, 'out', type);
    this.sql.exec('INSERT INTO adjacency (node_id, neighbour_id, edge_id, direction, edge_type) VALUES (?, ?, ?, ?, ?)', toId, fromId, id, 'in', type);
    return ok({ id, fromId, toId, type, properties, createdAt: nStr }, 201);
  }
  ...
  private updateEdge(id: string, body: UpdateEdgeBody): Response {
    ...
    const rows = [...this.sql.exec<EdgeRow>('SELECT * FROM edges WHERE id = ?', id)];
    if (!rows.length) return err('Edge not found', 404);
    const current = rowToEdge(rows[0]);
    const newType = type ?? current.type;
    const newProps = properties ? { ...current.properties, ...properties } : current.properties;
    this.sql.exec('UPDATE edges SET type = ?, properties = ? WHERE id = ?', newType, JSON.stringify(newProps), id);
    if (type && type !== current.type) {
      this.sql.exec('UPDATE adjacency SET edge_type = ? WHERE edge_id = ?', newType, id);
    }
    return ok({ ...current, type: newType, properties: newProps });
  }
  ...
  private getNeighbours(nodeId: string, url: URL): Response {
    ...
    return ok({ neighbours: adj.map(a => ({ node: nodeMap.get(a.neighbour_id)!, edge: edgeMap.get(a.edge_id)!, direction: a.direction })) });
  }

  private traverse(body: TraverseBody): Response {
    const { from, direction = 'out', maxDepth = 3, edgeTypes = [], nodeLabels = [], algorithm = 'bfs', limit = 200 } = body;
    ...
    return ok({ from, algorithm, direction, maxDepth, count: results.length, nodes: results });
  }

  private paths(body: PathsBody): Response {
    const { from, to, direction = 'out', edgeTypes = [], maxDepth = 10 } = body;
    if (!from || !to) return err('from and to required');
    if (from === to) return ok({ path: null, message: 'same node' });
    ...
    outer: while (queue.length > 0 && depth < maxDepth) {
      const levelSize = queue.length; depth++;
    ...
    if (!found) return ok({ path: null, message: `No path found` });
    ...
    const nodeMap  = new Map(nodeRows.map(n => [n.id, rowToNode(n)]));
    const edgeMap  = new Map(edgeRows.map(e => [e.id, rowToEdge(e)]));
    return ok({ path: { nodes: nodeIds.map(id => nodeMap.get(id)!), edges: edgeIds.map(id => edgeMap.get(id)!), length: edgeIds.length } });
  }

  private subgraph(body: SubgraphBody): Response {
    const { root, depth = 2, direction = 'out', edgeTypes = [], nodeLabels = [] } = body;
    ...
    const nodeRows = this.fetchByIds<NodeRow>('nodes', [...visitedNodes]);
    const edgeRows = this.fetchByIds<EdgeRow>('edges', [...visitedEdges]);
    const filteredNodes = nodeLabels.length ? nodeRows.filter(n => nodeLabels.includes(n.label)) : nodeRows;
    const filteredNodeIds = new Set(filteredNodes.map(n => n.id));
    const filteredEdges = edgeRows.filter(e => filteredNodeIds.has(e.from_id) && filteredNodeIds.has(e.to_id));
    const result: SubgraphResult = { nodes: filteredNodes.map(rowToNode), edges: filteredEdges.map(rowToEdge) };
    return ok({ ...result, nodeCount: result.nodes.length, edgeCount: result.edges.length });
  }

  private stats(): Response {
    const nodeCount = [...this.sql.exec<any>('SELECT COUNT(*) as c FROM nodes')][0]?.c ?? 0;
    const edgeCount = [...this.sql.exec<any>('SELECT COUNT(*) as c FROM edges')][0]?.c ?? 0;
    const labelRows = [...this.sql.exec<any>('SELECT label, COUNT(*) as count FROM nodes GROUP BY label ORDER BY count DESC')];
    const typeRows = [...this.sql.exec<any>('SELECT type, COUNT(*) as count FROM edges GROUP BY type ORDER BY count DESC')];
    return ok({ nodeCount, edgeCount, nodeLabels: labelRows, edgeTypes:  typeRows });
  }
  ...
  private queryAdjacency(nodeId: string, direction: 'in' | 'out' | 'both', edgeTypes: string[], limit: number): AdjacencyRow[] {
    const params: unknown[] = [nodeId];
    let sql = 'SELECT neighbour_id, edge_id, edge_type, direction FROM adjacency WHERE node_id = ?';
    if (direction !== 'both') { sql += ' AND direction = ?'; params.push(direction); }
    if (edgeTypes.length) { sql += ` AND edge_type IN (${edgeTypes.map(() => '?').join(',')})`; params.push(...edgeTypes); }
  ...
  private fetchByIds<T extends Record<string, SqlStorageValue>>(table: string, ids: string[]): T[] {
    ...
    const CHUNK = 100; const results: T[] = [];
...
function seg(path: string, idx: number): string { return path.split('/').filter(Boolean)[idx] ?? ''; }
